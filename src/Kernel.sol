 ...
 * @notice Central registry and permission manager for the Pars Protocol.
 * @dev    Based on the Default Framework architecture. The Kernel manages Modules (state)
 *         and Policies (logic), enabling modular upgrades without full redeployment.
 *
 * Persian terminology:
 * - Kernel = Hasteh (هسته) - Core/Nucleus
 * - Module = Bakhsh (بخش) - Section/Component
 * - Policy = Siasat (سیاست) - Policy/Strategy
 */
...
/// @dev    Amal (عمل) = Action in Persian
enum Actions {
    InstallModule,    // Nasb Module (نصب)
    UpgradeModule,    // Beruzsani Module (بروزرسانی)
    ActivatePolicy,   // Fa'alsazi Policy (فعال‌سازی)
    DeactivatePolicy, // Gheyf'alsazi Policy (غیرفعال‌سازی)
    ChangeExecutor,   // Taghir Mojri (تغییر مجری)
    MigrateKernel     // Mohajerat Kernel (مهاجرت)
}

/// @notice Used by executor to select an action and a target contract for a kernel action.
/// @dev    Dastur (دستور) = Instruction in Persian
struct Instruction {
    Actions action;
    address target;
}
...
/// @dev    Mojavez (مجوز) = Permission in Persian
struct Permissions {
    Keycode keycode;
    bytes4 funcSelector;
}

/// @notice 5-byte identifier for modules.
/// @dev    Ramz (رمز) = Code in Persian
...
/// @notice Convert bytes5 to Keycode type.
function toKeycode(bytes5 keycode_) pure returns (Keycode) {
    return Keycode.wrap(keycode_);
}

/// @notice Convert Keycode type to bytes5.
function fromKeycode(Keycode keycode_) pure returns (bytes5) {
    return Keycode.unwrap(keycode_);
}

/// @notice Ensure target address has code (is a contract).
function ensureContract(address target_) view {
    if (target_.code.length == 0) revert TargetNotAContract(target_);
}

/// @notice Ensure keycode contains only A-Z characters.
 ...
 * @dev    Motabegh (مطابق) = Adapter in Persian
 */
...
/**
 * @notice Base level extension of the kernel. Modules act as independent state components
 *         to be interacted with and mutated through policies.
 * @dev    Bakhsh (بخش) = Module/Section in Persian
 ...
 *         which are input from any address assigned as the executor. The executor can be changed.
 *
 *         Hasteh (هسته) = Kernel/Core in Persian
 */
contract Kernel {
    // =========  EVENTS ========= //
    ...
    /// @notice Address that is able to initiate Actions in the kernel.
    /// @dev    Mojri (مجری) = Executor in Persian
    ///         Can be assigned to a multisig or governance contract.
    ...
    /// @notice Mapping of a keycode to all of its policy dependents.
    /// @dev    Used to efficiently reconfigure policy dependencies.
    ...
    /// @dev    Ejra (اجرا) = Execute in Persian
    ...
    function _migrateKernel(Kernel newKernel_) internal {
        ...
        for (uint256 i; i < keycodeLen; ) {
            Module module_ = Module(getModuleForKeycode[allKeycodes[i]]);
            module_.changeKernel(newKernel_);
            unchecked {
                ++i;
            }
        }
